...
public class Utility
{
	...
	public static synchronized void getFields(String moduleAPIName, CommonAPIHandler handlerInstance) throws SDKException
	{
		...
		try
		{
			...
			else if (Initializer.getInitializer().getSDKConfig().getAutoRefreshFields())
			{
				...
				JSONObject apiSupportedmodules = getModules(null);
				...
				for (String module : apiSupportedmodules.keySet())
				{
					if (!recordFieldDetailsJson.has(module))
					{
						...
						JSONObject fieldDetails = (JSONObject) getFieldsDetails(apiSupportedmodules.getJSONObject(module).getString(Constants.API_NAME));
	...
	private static void modifyFields(String recordFieldDetailsPath, String modifiedTime) throws IOException, SDKException
	{
		...
		if (modifiedModules.length() > 0)
		{
			...
			for (String module : modifiedModules.keySet())
			{
				...
				Utility.getFields(moduleData.getString(Constants.API_NAME));
			}
		}
	}
		...
		for (int index = 0; index < modulerelatedListJA.length(); index++)
		{
			...
			if (relatedListJO.getString(Constants.API_NAME) != null && relatedListJO.getString(Constants.API_NAME).equalsIgnoreCase(relatedModuleName))
			{
				...
				if(!relatedListJO.getString(Constants.MODULE).equalsIgnoreCase(Constants.NULL_VALUE))
				{
					...
					Utility.getFields(relatedListJO.getString(Constants.MODULE), commonAPIHandler);
				}
	...
	public static JSONObject searchJSONDetails(String key)
	{
		...
		while (iter.hasNext())
		{
			...
			if (keyInJSON.equalsIgnoreCase(key))
			{
				...
				returnJSON.put(Constants.MODULEDETAILS, Initializer.jsonDetails.getJSONObject(keyInJSON));
	...
	public static synchronized boolean verifyPhotoSupport(String moduleAPIName) throws SDKException
	{
		try
		{
			...
			if(modules.optJSONObject(moduleAPIName.toLowerCase()) != null)
			{
				...
				if(!moduleMetaData.getString(Constants.GENERATED_TYPE).equals(Constants.GENERATED_TYPE_CUSTOM))
				{
	...
	private static JSONObject getModuleNames() throws Exception
	{
		...
		if(!recordFieldDetails.exists() || (recordFieldDetails.exists() && Initializer.getJSON(recordFieldDetailsPath).optJSONObject(Constants.SDK_MODULE_METADATA) == null))
		{
	...
	private static void writeModuleMetaData(String recordFieldDetailsPath, JSONObject moduleData) throws IOException
	{
		...
		fieldDetailsJSON.put(Constants.SDK_MODULE_METADATA, moduleData);
	...
	private static void setDataType(JSONObject fieldDetail, Field field, String moduleAPIName) throws SDKException
	{
		...
		if(Constants.KEY_VS_INVENTORY_MODULE.containsKey(keyName) && moduleAPIName.toLowerCase().equalsIgnoreCase(Constants.KEY_VS_INVENTORY_MODULE.get(keyName)))
		{
		...
		if(apiType.toLowerCase().equalsIgnoreCase(Constants.MULTI_SELECT_LOOKUP))
		{
			...
			if(field.getMultiselectlookup() != null)
			{
				...
				{
					String linkingModule = field.getMultiselectlookup().getLinkingModule();
		...
		if(apiType.toLowerCase().equalsIgnoreCase(Constants.MULTI_USER_LOOKUP))
		{
			...
			if(field.getMultiuserlookup() != null)
			{
				...
				{
					String linkingModule = field.getMultiuserlookup().getLinkingModule();
